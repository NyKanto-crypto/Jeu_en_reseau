...
import game.Player;
import game.Table;
import listener.Listener2;

public class Window2 extends JFrame {
    Table table = new Table();

    public Window2(Table table) {
        this.table = table;
        // LES JOUEURS
        ...
        for (int i = 0; i < this.table.getButtons().length; i++) {
            for (int j = 0; j < this.table.getButtons()[0].length; j++) {
                ...
                this.table.getButtons()[i][j].addActionListener(new Listener2(title_label, this.table));
            }
        }
